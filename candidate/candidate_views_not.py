...
def parse_form_array(request, prefix, fields):
    """Helper to parse dynamic form arrays (e.g., Experience, Education)."""
    items = []
    for key in request.POST:
        if key.startswith(f"{prefix}["):
            ...
            while len(items) <= idx:
                items.append({})
            field_name = key.split('[')[2].strip(']')
            if field_name in fields:
                items[idx][field_name] = request.POST[key]
    return [item for item in items if all(field in item for field in fields)]
...
def submit_resume(request):
    try:
        ...
        processed_data = settings.MONGO_DB[PROCESSED_RESUMES_COLLECTION].find_one({'original_pdf_id': pdf_id})
        ...
        resume_data = {
            ...
            'Experience': parse_form_array(request, 'Experience', ['Job_Title', 'Company_Name', 'Dates_Duration']),
            'Education': parse_form_array(request, 'Education', ['Degree', 'Institution', 'Dates']),
            'Projects': parse_form_array(request, 'Projects', ['Name', 'Description']),
        ...
        if resume_data['Personal_Information']['LinkedIn'] and not re.match(r'^(https?:\/\/)?([\w\d-]+\.)+[\w\d-]+(\/.*)?$', resume_data['Personal_Information']['LinkedIn']):
            logger.warning(f"Invalid LinkedIn URL: {resume_data['Personal_Information']['LinkedIn']}")
        ...
        # Find the corresponding temp application
        temp_application = settings.MONGO_DB[TEMP_APPLICATIONS_COLLECTION].find_one({
            'user_id': user_id,
            'resume_id': pdf_id
        })
        if not temp_application:
            logger.error(f"No temp application found for user_id: {user_id}, pdf_id: {pdf_id}")
            return render(request, 'candidate/error.html', {'message': 'Application not found'})

        job_id = temp_application['job_id']

        # Save to processed_resumes
        settings.MONGO_DB[PROCESSED_RESUMES_COLLECTION].update_one(
        ...
        updated_document = settings.MONGO_DB[PROCESSED_RESUMES_COLLECTION].find_one(
            {'original_pdf_id': pdf_id, 'user_id': user_id}
        )

        if not job or not updated_document:
            logger.error(f"Job {job_id} or resume {pdf_id} not found")
            return render(request, 'candidate/error.html', {'message': 'Job or resume not found'})
        ...
        resume_data = updated_document.get("resume_data", {})
        resume_data["_id"] = {"$oid": str(updated_document["_id"])}
        ...
        application = {
            ...
            'application_data': {
                'submission_method': SUBMISSION_METHOD_MANUAL,
                'submitted_at': datetime.now()
            }
        }

        # Run evaluation and link to application
        evaluation_df = test_evaluations(resume_data, job_data, application_id=str(temp_application["_id"]))
        evaluation_id = settings.MONGO_DB[RESUMES_EVALUATION_COLLECTION].find_one({
            "application_id": str(temp_application["_id"])
        })["_id"]
        application["evaluation_id"] = str(evaluation_id)

        # Save application
        settings.MONGO_DB[APPLICATIONS_COLLECTION].insert_one(application)

        # Increment application count in jobs
        ...
        settings.MONGO_DB[TEMP_APPLICATIONS_COLLECTION].delete_one({'_id': temp_application['_id']})

        logger.info(f"Resume submitted for user {user_id}, pdf_id {pdf_id}, job_id {job_id}")
...
#             test_evaluations(resume_data, job_data, application_id=str(temp_application["_id"]))
