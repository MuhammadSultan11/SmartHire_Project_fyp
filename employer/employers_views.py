...
def send_interview_invitation(request):
    try:
        ...
        interview_type = data.get('interview_type', 'Video Call')
        duration_minutes = int(data.get('duration_minutes', 30))
        deadline = datetime.strptime(data.get('deadline'), '%Y-%m-%d') if data.get('deadline') else None
        custom_message = data.get('custom_message', '')
        
        if not all([application_id, interview_round, deadline]):
            return JsonResponse({'success': False, 'error': 'Missing required fields'}, status=400)
        
        application = settings.MONGO_DB['applications'].find_one({'_id': ObjectId(application_id)})
        if not application:
            return JsonResponse({'success': False, 'error': 'Application not found'}, status=404)
        
        if application['posted_by'] != request.mongo_user['user_id']:
            return JsonResponse({'success': False, 'error': 'Unauthorized'}, status=403)
        
        # Check if an invitation already exists
        existing_invitation = settings.MONGO_DB['interview_invitations'].find_one({
            'application_id': application_id,
            'status': {'$in': ['pending', 'accepted']}
        })
        if existing_invitation:
            return JsonResponse({'success': False, 'error': 'An active interview invitation already exists'}, status=400)
        
        invitation_document = {
            'application_id': application_id,
            'user_id': application['user_id'],
            'job_id': application['job_id'],
            'posted_by': application['posted_by'],
        ...
        result = settings.MONGO_DB['interview_invitations'].insert_one(invitation_document)
        invitation_id = str(result.inserted_id)
        
        # Log activity
        settings.MONGO_DB['user_activity_logs'].insert_one({
            'user_id': request.mongo_user['user_id'],
        ...
        return JsonResponse({'success': True, 'invitation_id': invitation_id})
    except Exception as e:
        logger.error(f"Error in send_interview_invitation: {str(e)}", exc_info=True)
